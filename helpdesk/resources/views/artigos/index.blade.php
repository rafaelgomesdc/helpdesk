    ...
    @else
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td style="color:var(--text-primary); font-weight:500;">{{ Str::limit($artigo->titulo, 60) }}</td>
                    <td>{{ $artigo->categoria?->nome ?? '—' }}</td>
                    <td>{{ $artigo->autor?->name ?? '—' }}</td>
                    <td style="font-family:'IBM Plex Mono',monospace; font-size:11px;">{{ $artigo->created_at->format('d/m/Y') }}</td>
                    <td>
