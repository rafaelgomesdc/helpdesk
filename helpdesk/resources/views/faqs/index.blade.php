    ...
    @else
        <table>
            ...
            <tbody>
                @foreach($faqs as $faq)
                <tr>
                    <td style="color:var(--text-primary); font-weight:500;">{{ Str::limit($faq->pergunta, 80) }}</td>
                    <td>{{ $faq->categoria?->nome ?? '—' }}</td>
                    <td style="font-family:'IBM Plex Mono',monospace; font-size:11px;">{{ $faq->created_at->format('d/m/Y') }}</td>
                    <td>
