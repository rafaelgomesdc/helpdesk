...
<div class="form-card">
    <form method="POST" action="{{ route('faqs.update', $faq) }}">
        ...
        <div class="form-group">
            ...
            <select name="categoria_id" class="form-select">
                ...
                @foreach($categorias as $cat)
                    <option value="{{ $cat->id }}" {{ old('categoria_id', $faq->categoria_id) == $cat->id ? 'selected' : '' }}>{{ $cat->nome }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <textarea name="pergunta" class="form-textarea" rows="3" required>{{ old('pergunta', $faq->pergunta) }}</textarea>
            @error('pergunta') <div class="form-error">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label class="form-label">Resposta *</label>
            <textarea name="resposta" class="form-textarea" rows="7" required>{{ old('resposta', $faq->resposta) }}</textarea>
